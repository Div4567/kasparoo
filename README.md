...
3. **AI Service (Python + FastAPI):** Handles Intent Classification, queries the Vector DB for context (RAG), and prompts the LLM (Qwen via Ollama) to generate accurate, non-hallucinated responses.
4. **Data Layer:** 
...
- **Node.js** (v18+)
- **Python** 3.10+
- **Ollama** (with local `qwen2.5:3b` model downloaded: `ollama run qwen2.5:3b`)
...
- **Model Switching:** Easily configurable to swap out Qwen for other models served via Ollama or remote APIs via the `LLM_MODEL` environment variable.
